...
class ApplicationController extends Controller
{
     ...
     public function Wechatpay($price)
  {
	 ...
	 //支付宝免签约接口 by zfbjk.com
     public function alidirectpay($price)
	 {
		 $user = User::where('id_wechat', $this->tel)->first();
		 $balance = $user->body_balance;
		 echo "<form action='/alidirectV2/index.php' method='post' name='alidirect'>
		 <input type='hidden' name='title' value='".$this->tel."' />
		 <input type='hidden' name='payAmount' value='".$price."' />
		 <input type='hidden' name='balance' value='".$balance."' />
		 <script>document.alidirect.submit();</script>";
	 }
	 public function alidirectcheck()
	 {
		 $user = isset($_POST['user'])?$_POST['user']:'';
		 $balance = isset($_POST['balance'])?$_POST['balance']:'';
		 $user = User::where('id_wechat', $user)->first();
		 $curbalance = $user->body_balance;
		 if($curbalance>$balance)
		 {
			 exit("success");
		 }
	 }
	 public function alidirect()
	 {
		 //您在www.zfbjk.com的商户ID
		 $alidirect_pid = "19813";
		 //您在www.zfbjk.com的商户密钥
		 $alidirect_key = "f9b01e36fea283d13497d21dcd013e91";
		 ...
		 if(strtoupper(md5($alidirect_pid . $alidirect_key . $tradeNo . $Money . $title . $memo)) == strtoupper($Sign))
		 {
			 ...
			 $user = User::where('id_wechat', $title)->first();
			 if($user)
			 {
				 $order = payRequest::where('body_transfer_number',$tradeNo)->first();
				 if($order){exit("success");}
				 $payRequest = new payRequest;
				 $payRequest->id_user = $title;
				 $payRequest->body_stake = intval($Money);
				 $payRequest->body_gateway = 'ALIPAY';
				 $payRequest->body_transfer_number = $tradeNo;
...
public function postSMS()
  {
    ...
    if ($type == 1) {
      ...
      session(['code' => $code]);
      $body = urlencode("【熙桓贸易】尊敬的用户，您的验证码是 " . $code . "，请在10分钟内输入。");
