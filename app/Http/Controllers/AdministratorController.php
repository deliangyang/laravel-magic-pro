...
class AdministratorController extends Controller
{
    ...
    {
        $this->is_admin = $request->session()->get('is_admin');
    }
    ...
    public function users(Request $request)
    {
        ...
        if (!$this->is_admin) {
            $datas->where('id_introducer', $request->session()->get('administrator'));
        }
    ...
    public function orders(Request $request)
    {
        ...
        if (!$this->is_admin) {
            $ids = array();
            $datas = User::where('id_introducer', $request->session()->get('administrator'))->get();
            foreach ($datas as $user) {
                array_push($ids, $user->id_wechat);
            }
        }
        $datas = Order::with('user')->orderBy('created_at', 'desc');
        if (!$this->is_admin) {
            $datas->whereIn('id_user', $ids);
        }
...
//    dd($datas);
    ...
    public function signIn(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            if (filter_var($request->input('email'), FILTER_VALIDATE_EMAIL)) {
                ...
                // 手机号码登陆, 用户
                $agent = User::where('id_wechat', $request->input('email'))->where('password', md5($request->get('password')))->first();
                if ($agent) {
                    $request->session()->put('administrator', $agent->id_wechat);
    ...
    public function usersExport(Request $request)
    {
        ...
        if ($this->is_admin) {
            ...
            $datas = User::whereIn('id_wechat', $ids)->get();
        }
    ...
    public function ordersExport(Request $request)
    {
        ...
        if ($this->is_admin) {
            ...
            $datas = Order::whereIn('id_user', $ids)->get();
        }
        foreach ($datas as $item) {
            ...
            $result[] = array(
                $item->id,
                isset($item->user->body_phone) ? $item->user->body_phone  : 0,
    ...
    public function recordsExport(Request $request)
    {
        ...
        if ($this->is_admin) {
            ...
            $datas = Record::whereIn('id_user', $ids)->get();
        }
    ...
    public function payRequestsExport(Request $request)
    {
        ...
        if ($this->is_admin) {
            ...
            $datas = PayRequest::whereIn('id_user', $ids)->get();
        }
    ...
    public function withdrawRequestsExport(Request $request)
    {
        ...
        if ($this->is_admin) {
            ...
            $datas = WithdrawRequest::whereIn('id_user', $ids)->get();
        }
