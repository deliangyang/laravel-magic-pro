...
//        $line->save();
            ...
            if (strstr($item, '=')) {
                ...
                if ($objectName && $object = Object::where('body_tag', $objectName)->first()) {
                    if ($objectName == 'sh000010'
                        ...
                        $weidiao = (mt_rand(1, 9) / 100);
                        $objectPrice = str_replace('"', '', $arrayForObject[0]);
                        $objectPriceTime = $arrayForObject[12] . ' ' . $arrayForObject[6];
                        $objectPriceMin = $arrayForObject[5] + $weidiao;
                        $objectPriceMax = $arrayForObject[4] + $weidiao;
    ...
    protected function computeNetwork($user, $order)
    {
        ...
        while ($user->id_introducer != 0) {
            ...
            if ($introducerIndex <= 3) {
                ...
                $introducer->body_balance = floatval($introducer->body_balance) + $bonus * 0.1;
    ...
    public function orderCreate(Request $request, Response $response)
    {
        ...
        $user->body_balance = floatval($user->body_balance) - $request->input('stake') * (1 + 0.02);
        ...
        if ($user->body_balance < 0) {
            ...
            $record->id_user = $user->id_wechat;
            $record->id_order = $order->id;
            $record->body_name = $request->input('direction') == 1 ? '買入看漲手续费' : '買入看跌手续费';
            $record->body_direction = 0;
            $record->body_stake = $order->body_stake * 0.02;
            $record->save();
        }
        if (!$order) {
            return response()->json([
